...
The power supply section generates the required regulated rails. A 3.3 V switching regulator (AP63203WU-7) provides the logic supply used by the PIC18F57Q83 microcontroller, control inputs, and other digital circuitry. A separate 6 V linear regulator (L7806ABD2T) supplies power input for the motors. Decoupling capacitors are included around the regulators and major ICs to reduce electrical noise and ensure stable voltage levels.
